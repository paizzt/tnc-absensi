            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-body p-4">
                    ...
                    @if($permission->document_path)
                        <a href="{{ asset('storage/' . $permission->document_path) }}" target="_blank" class="btn btn-outline-primary w-100">Lihat</a>
                    @else
                ...
                <div class="card-body p-4">
                    ...
                    <div class="d-grid gap-2">
                        <form action="{{ route('teacher.permissions.approve', $permission->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-success w-100 fw-medium">Setujui</button>
                        </form>
                        ...
                        <form action="{{ route('teacher.permissions.reject', $permission->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menolak permohonan ini?');">
                            @csrf
                            <button type="submit" class="btn btn-danger text-white bg-opacity-75 w-100 fw-medium border-0">Tolak</button>
                        </form>
                    </div>
                </div>
            </div>
